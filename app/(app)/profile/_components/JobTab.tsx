...
import { Pencil, Briefcase, Building2, UserCircle, Map, Zap, Calendar, UserCheck } from 'lucide-react'
  ...
  return (
    <Card className="bg-slate-900/40 border-slate-800 backdrop-blur-sm shadow-2xl relative overflow-hidden">
      ...
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-800/50 bg-slate-900/20 px-8 py-6">
        <div>
          <CardTitle className="text-lg font-black uppercase tracking-widest text-blue-400">Operational Status</CardTitle>
          <CardDescription className="text-slate-500 uppercase text-[10px] font-black tracking-[0.2em]">Deployment and organizational parameters</CardDescription>
        </div>
        <Button 
          size="icon" 
          variant="outline" 
          className="border-slate-800 hover:bg-blue-600 hover:text-white transition-all h-10 w-10 rounded-lg"
          onClick={() => window.dispatchEvent(new CustomEvent('open-edit-modal', { detail: 'job' }))}
        >
      ...
      <CardContent className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Tile
            icon={Map}
            label="Work Vector"
            value={(user as any)?.workMode || '-'}
            accentColor="bg-red-600"
          />
          <Tile
            icon={Zap}
            label="Engagement Type"
            value={(user as any)?.jobType || '-'}
            accentColor="bg-amber-600"
          />
          <Tile
            icon={UserCheck}
            label="Command Authority"
            value={(user as any)?.reportingTo || '-'}
